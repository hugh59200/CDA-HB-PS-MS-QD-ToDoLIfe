...
async function creationJour(titre, humeur, texte, setshowList) {
	const id = localStorage.getItem('id');
...
function timeOut(time, status, setshowList) {
	setTimeout(() => {
		popUpSuccess(status, setshowList);
	}, time);
}



function popUpSuccess(status, setshowList) {
	if (status === 200) {
		return ajoutOk(setshowList);
	}
	if (status === 409) {
		return ajoutKo();
	}
